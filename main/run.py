...
class CNN3(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.features = nn.Sequential(
            ...
            nn.PReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(in_channels=256, out_channels=128, kernel_size=7, stride=1),
            nn.BatchNorm2d(num_features=128),
            nn.PReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(in_channels=128, out_channels=192, kernel_size=5, stride=1),
            nn.BatchNorm2d(num_features=194),
            nn.PReLU(),
